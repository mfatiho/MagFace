...
parser.add_argument('--feat-indexes', type=str,
                    help='selection feat indexes')
...
def perform_1v1_eval(args):
    ...
    indexes_file = args.feat_indexes
    ...
    feat_indexes = np.load(f"indexes/{indexes_file}")
    #print(f"\n{feat_indexes.shape[0]} Features Selected From {indexes_file}")
    for k, v in feat_pairs.items():
        ...
        if feat_indexes is not None:
            np_feat_a = np_feat_a[feat_indexes]
            np_feat_b = np_feat_b[feat_indexes]
    ...
    acc = 'Accuracy: %2.5f+-%2.5f' % (np.mean(accuracy)*100, np.std(accuracy)*100)
    print(f"{os.path.basename(args.feat_list)}, {indexes_file}, {feat_indexes.shape[0]}, {acc}")
